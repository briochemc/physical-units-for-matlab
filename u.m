...
properties (Constant = true)
    ...
    Ao = scd(1e-10*u.m,'Ao') % �ngstr�m
    ang = scd(u.Ao,'ang') % �ngstr�m
    ...
    kyr = scd(1e3*u.yr,'kyr') % julian year
    Myr = scd(1e6*u.yr,'Myr') % julian year
    ...
    mole = scd(u.mol,'mole')
    mmol = scd(u.milli * u.mol, 'mmol')
    umol = scd(u.micro * u.mol, 'umol')
    nmol = scd(u.nano * u.mol, 'nmol')
    pmol = scd(u.pico * u.mol, 'pmol')
    kmol = scd(u.kilo * u.mol, 'kmol')
    Mmol = scd(u.mega * u.mol, 'Mmol')
    Gmol = scd(u.giga * u.mol, 'Gmol')
    Tmol = scd(u.tera * u.mol, 'Tmol')
    kat = scd(u.mol/u.s,'kat') % katal
    katal = scd(u.kat,'katal')
    M = scd(u.mol/u.L,'M') % molar
    mM = scd(u.milli * u.M, 'mM')
    uM = scd(u.micro * u.M, 'uM')
    nM = scd(u.nano * u.M, 'nM')
    pM = scd(u.pico * u.M, 'pM')
    kM = scd(u.kilo * u.M, 'kM')
    MM = scd(u.mega * u.M, 'MM')
    GM = scd(u.giga * u.M, 'GM')
    TM = scd(u.tera * u.M, 'TM')
    molar = scd(u.M,'molar')
